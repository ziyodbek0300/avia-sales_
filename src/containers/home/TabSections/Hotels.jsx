    ...
    return (<div className={'bg-white p-2 gap-5 shadow border rounded-lg'}>
            <div onClick={handlePress} className="cursor-pointer flex gap-6"
                ...
                <div className={"flex flex-col justify-between"}>
                    ...
                    <p className={"mt-auto text-2xl"}>Цена:
                        ${Array.isArray(e.price) && e.price.length > 0 && Math.floor(e.price[0].price)}</p>
                </div>
                {hotelsTownLists.map(a => {
                    return a.id === e.town && (<p className="text-sm block bg-red-500">{a.title}</p>)
                })}
            </div>
            ...
            {values.open ? (<>
                ...
                <div className={"bg-gray-200 rounded p-5"}>
                    {values.tabIndex === 0 ? (<div className={"mt-16"}>
                        <form action="">
                            <div>
...
function Hotels() {
    ...
    const [values, setValues] = useState({
        town: hotelsTownLists[0].id, datebeg: null, dateend: null
    })
